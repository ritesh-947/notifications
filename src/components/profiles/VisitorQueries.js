  ...
  useEffect(() => {
      const fetchDecryptedMessages = async () => {
        try {
          ...
          const response = await axios.get('https://promo-server-5iob.onrender.com/api/creator/queries', {
          // const response = await axios.get('http://localhost:5011/api/creator/queries', {
            headers: {
  ...
  try {
    ...
    const statuses = await Promise.all(
      queryIds.map(async (queryId) => {
        const response = await axios.get(`https://promo-server-5iob.onrender.com/api/creator/message-like-status/${queryId}`, {
        // const response = await axios.get(`http://localhost:5011/api/creator/message-like-status/${queryId}`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Session ${sessionId}`, // Use sessionId for authentication
          },
    ...
    console.error('Error fetching like statuses:', error.response?.data || error.message);
    setLoading(false);
  }
};
...
useEffect(() => {
  const fetchReplies = async () => {
    try {
      ...
      if (!sessionId) {
        console.error('Session ID is missing.');
        setReplies([]); // Ensure replies are cleared if sessionId is missing
        setLoading(false);
        return;
      }
...
const handleReplySubmit = async (queryId, isEdit = false) => {
  ...
  try {
    ...
    if (!sessionId) {
      console.error('Session ID is missing.');
      return; // Stop further execution if sessionId is not available
    }
    ...
    const response = await axios.post(
      `https://promo-server-5iob.onrender.com${endpoint}`,
...
const handleDeleteReply = async (queryId) => {
  ...
  try {
    ...
    if (!sessionId) {
      console.error('Session ID is missing.');
      return; // Exit early if sessionId is not available
    }

    const response = await axios.delete(`https://promo-server-5iob.onrender.com/api/creator/delete-reply/${queryId}`, {
    // const response = await axios.delete(`http://localhost:5011/api/creator/delete-reply/${queryId}`, {
      headers: {
        Authorization: `Session ${sessionId}`, // Use sessionId for authentication
      },
    });
    ...
    if (response.status === 200) {
      ...
      console.log(`Reply with Query ID: ${queryId} successfully deleted.`);
    }
  } catch (error) {
...
const handleFavoriteClick = async (queryId) => {
  ...
  try {
    ...
    await axios.post(
      'https://promo-server-5iob.onrender.com/api/creator/toggle-message-like',
      // 'http://localhost:5011/api/creator/toggle-message-like',
      {
        query_id: queryId,
    ...
    console.log(`Favorite status toggled for Query ID: ${queryId}, New Status: ${newFavoriteStatus}`);
  } catch (error) {
...
const handleReportSubmit = async (reason) => {
  ...
  try {
    const sessionId = localStorage.getItem('sessionId');
    if (!sessionId) {
      alert('Session ID is missing. Please log in to report a query.');
      return; // Exit early if sessionId is not available
    }

    const response = await axios.post(
      'https://promo-server-5iob.onrender.com/api/visitor/report-query',
    ...
    >
      ...
      <Box display="flex" justifyContent="flex-end" alignItems="center" sx={{ mt: -3, ml: 1, width: '100%' }}>
       <Typography
        ...
        sx={{ fontStyle: 'italic', mt:-1.8, mr:-7}}
      >
        
      
        {reply.reply_at ? `At ${formatDateTime(reply.reply_at)}` : ''}
